...
- 각각의 기능을 메서드로 만든 덕분에 각각의 기능이 모듈화가 됨. 따라서 아래와 같은 장점이 생김.
    1. 중복 제거 : 로직의 중복이 제거됨.
    -> 같은 로직이 필요하면 해당 메서드를 여러번 호출하면 됨. 
...
- 모듈화 - 레고 블럭을 생각하면 편함.
    - 레고에서 필요한 블럭을 가져다 꼽아서 사용할 수 있듯이  음악 플레이어의 기능이 필요하면 해당 기능을 메서드의 호출만으로 손쉽게 사용할 수 있음.
...
public class MusicPlayerMain3 {
    ...
    static void musicPlayerVolume(MusicPlayerData mp, String type) {
        if (type.equals("+"))
            mp.volume++;
        else
            mp.volume--;
